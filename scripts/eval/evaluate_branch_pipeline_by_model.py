...
class FunctionModel:
    ...
    예:
    experiments/models/sin/SVR.joblib
    experiments/models/sin/SVR_y_scaler.joblib
    """
...
class FunctionModelRegistryByModel:
    """
    특정 base_model에 대해 모든 함수 모델을 로드한다.
    ...
    def __init__(self, model_root: str, base_model: str):
    ...
    def load_all(self):
        ...
        for function_name in os.listdir(self.model_root):
            ...
            model_path = os.path.join(function_dir, f"{self.base_model}.joblib")
            scaler_path = os.path.join(function_dir, f"{self.base_model}_y_scaler.joblib")
            ...
            if os.path.exists(model_path) and os.path.exists(scaler_path):
                try:
                    ...
                    print(f"[WARN] {function_name} - {self.base_model} 모델 로드 실패: {e}")
    ...
    def predict(self, function_name: str, x: float) -> float:
        ...
        if function_name not in self.models:
            raise ValueError(
                f"'{function_name}' 함수의 '{self.base_model}' 모델이 없습니다. "
...
def solve_principal_only(
    chain: List[str],
    constant: float,
    function_registry: FunctionModelRegistryByModel,
    x_search_min: float,
    x_search_max: float
) -> Dict[str, Any]:
...
def evaluate_dataset_by_model(
    ...
    print("[1] Loading interval predictor...")
    ...
    for base_model in model_names:
        print(f"\n[3] Evaluating base_model={base_model}")

        function_registry = FunctionModelRegistryByModel(
            model_root=function_model_root,
            base_model=base_model
        ).load_all()

        print(f"Loaded functions for {base_model}: {function_registry.loaded_functions()}")

        for idx, row in df.iterrows():
            if (idx + 1) % 100 == 0:
                print(f"  {base_model}: {idx + 1}/{len(df)}")
    ...
    detailed_path = os.path.join(output_dir, "branch_pipeline_detailed_results_by_model.csv")
    result_df.to_csv(detailed_path, index=False, encoding="utf-8-sig")

    summary_model_method = make_group_summary(
        result_df,
        group_cols=["base_model", "method"]
    )
    summary_model_method_path = os.path.join(output_dir, "summary_by_model_method.csv")
    summary_model_method.to_csv(summary_model_method_path, index=False, encoding="utf-8-sig")

    summary_test_type = make_group_summary(
        result_df,
        group_cols=["test_type", "base_model", "method"]
    )
    summary_test_type_path = os.path.join(output_dir, "summary_by_test_type_model_method.csv")
    summary_test_type.to_csv(summary_test_type_path, index=False, encoding="utf-8-sig")

    summary_root_region = make_group_summary(
        result_df,
        group_cols=["root_region", "base_model", "method"]
    )
    summary_root_region_path = os.path.join(output_dir, "summary_by_root_region_model_method.csv")
    summary_root_region.to_csv(summary_root_region_path, index=False, encoding="utf-8-sig")

    summary_depth = make_group_summary(
        result_df,
        group_cols=["depth", "base_model", "method"]
    )
    summary_depth_path = os.path.join(output_dir, "summary_by_depth_model_method.csv")
    summary_depth.to_csv(summary_depth_path, index=False, encoding="utf-8-sig")

    print("\nSaved results:")
    print(f"  detailed: {detailed_path}")
    print(f"  summary by model/method: {summary_model_method_path}")
    print(f"  summary by test type/model/method: {summary_test_type_path}")
    print(f"  summary by root region/model/method: {summary_root_region_path}")
    print(f"  summary by depth/model/method: {summary_depth_path}")
...
def main():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="experiments_branch_eval_by_model"
    )

    parser.add_argument(
        "--models",
        type=str,
        default="SVR,DecisionTreeRegressor,RandomForestRegressor"
    )
